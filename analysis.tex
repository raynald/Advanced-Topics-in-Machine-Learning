...
\begin{frame}{Basic Pegasos Algorithm (SGD)}
    ...
    \[
        \E[f(\overline{\wv})]\le f(w^*) + {\color{red} c} \times \frac{1+\ln(T)}{2\lambda T}
    \]
    where $c=4R^2$.
\end{frame}
...
\begin{frame}{Run-Time of Pegasos}
    \begin{itemize}
        ...
        \item Previous, depends on ``difficulty'' of problem (both $\lambda$ and $\epsilon$)
    \end{itemize}
\end{frame}
...
\begin{frame}{Analysis from Lacoste}
    \begin{itemize} 
        \item Classical analysis: $\eta_t=\frac{1}{\lambda t}$
            \begin{itemize}
                \item $\E f\!\left(\frac1T\sum_{t=1}^T \wv^{(t)} \right)- f(\wv^*) \le \frac{2X^2R^2}{\lambda T}(\ln{T}+1)$
                \item For Hinge loss $X = 1$, the result is same as before.
            \end{itemize}
        \item New analysis: $\eta_t=\frac{2}{\lambda (t+1)}$
            \begin{itemize}
                \item $\E f(\frac{2}{T(T+1)}\sum_{t=1}^{T}t w^{(t)}) - f(w^*) \le \frac{8X^2 R^2}{\lambda (T+1)}$
                \item $\E_{i(T)} \left[  \|\wv^{(T+1)} - \wv^*\|^2  \|  \wv^t  \right]  \le \frac{16X^2 R^2}{\lambda^2 (T+1)}$

...
\begin{frame}{Lemma}
With the Lipschitz assumption on $l(y,u)$ that $\|l'(y,u)\| \le X$, and assuming that $\|\x_i\|\le     R$ $\forall i$ where $i$ is picked according to $p_i$, it holds that
\[
...
\begin{frame}
    ...
    \end{block}
    \[
        \sqrt{\E_{i^{(t)}} \|\chiv_i^{(t)}\|^2} \le \sqrt{\E_{i^{(t)}} \| l'\xv_i \|^2} + \lambda           \sqrt{\E_{i^{(1:t-1)}} \| \wv^{(t)} \|^2} \le XR + 
    \] 
    \[
    ...
    \]
    \[
        \le (1-\lambda \eta_t)\sqrt{\E_{i(t)} \| \wv^{(t)}\|^2} +         \lambda \eta_t \frac{XR}{\lambda}.
    \]
...
\begin{frame}{Stochastic Dual Coordinate Ascent}
    \begin{itemize}
        \item Dual problem
            \[
                max_\theta D(\theta) := \frac1n \sum_{i=1}^n -f_i^*(-\theta_i) - \lambda r^*(\frac{1}{\lambda n}\sum_{i=1}^n \theta_i).
            \]
        \item Relationship with primal variable: $\wv=\nabla r^*(\vv(\theta))$, $\vv(\theta)=\frac{1}{\lambda n} \sum_{i=1}^n \theta_i$
